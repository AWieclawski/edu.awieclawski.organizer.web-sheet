...
public class MonthRecordService extends BaseService<MonthRecord, MonthRecordDto> {
    ...
    private void assignDateCells(MonthRecordDto monthRecordDto) {
        if (monthRecordDto.getCreated() != null) {
            List<DateCellDto> dateCellDtos = dateCellService.getDateCellsByMonthRecord(monthRecordDto.getCreated());
            ReflectionUtils.setFieldValue(monthRecordDto, "dateCellsList", dateCellDtos, true);
        }
    }
