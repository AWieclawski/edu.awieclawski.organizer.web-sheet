...
import edu.springboot.organizer.data.daos.base.helpers.JdbcInsertHelper;
import edu.springboot.organizer.data.daos.base.helpers.JdbcUpdateHelper;
...
public abstract class BaseDao<S extends BaseEntity, T extends BaseDto> {
    ...
    List<T> executeUpdateDtos(List<T> dtos) throws InstantiationException {
        ...
        JdbcUpdateHelper<S, T> jdbcUpdateHelper = ((JdbcUpdateHelper<S, T>) getEntityUpdate()
                .withTableName(getTableName()))
                .withIdColumnName(BaseEntity.BaseConst.ID.getColumn())
                .withBatchSize(50)
                .withRawMapper(getBaseRowMapper());

        int[][] result = jdbcUpdateHelper.executeBatchUpdate(dtos);
    ...
    List<T> executeInsertDtos(List<T> dtos) throws InstantiationException {
        ...
        @SuppressWarnings("unchecked")
        JdbcInsertHelper<S, T> simpleJdbcUpdate = ((JdbcInsertHelper<S, T>) getExtendedJdbcInsert()
                .withTableName(getTableName()))
                .withBatchSize(50)
                .withRawMapper(getBaseRowMapper())
    ...
    SimpleJdbcInsert getEntityInsert() throws InstantiationException {
    ...
    private JdbcUpdateHelper<S, T> getEntityUpdate() throws InstantiationException {
        DataSource dataSource = getDataSource();
        if (dataSource != null) {
            return new JdbcUpdateHelper<S, T>(dataSource);
        }
        throw new InstantiationException("No DataSource instantiation!");
    }

    private JdbcInsertHelper<S, T> getExtendedJdbcInsert() throws InstantiationException {
        DataSource dataSource = getDataSource();
        if (dataSource != null) {
            return new JdbcInsertHelper<>(dataSource);
        }
        throw new InstantiationException("No DataSource instantiation!");
    }
    ...
    private int executeEntityUpdate(Map<String, Object> entityParameters) throws InstantiationException {
        ...
        if (idObjectValue != null) {
            ...
            return jdbcUpdateHelper.executeUpdate(entityParameters);
        }
